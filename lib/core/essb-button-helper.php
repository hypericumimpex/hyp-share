		...
		foreach ( $networks_order as $single ) {
			...
			if (in_array ( $single, $networks )) {
				...
				$content .= sprintf('<li class="essb_item essb_link_%8$s nolightbox%2$s">
						%12$s<a href="%3$s" title="%4$s" onclick="%5$s" target="%10$s" rel="%11$s" class="nolightbox" %7$s>%13$s<span class="essb_icon essb_icon_%17$s"></span><span class="essb_network_name%9$s">%14$s%6$s%15$s</span>%18$s</a>%16$s</li>',
	...
	switch ($network) {
		...
		case 'vk' :
			if (ESSBGlobalSettings::$vkontakte_fullshare) {
				...
				$url = sprintf ( 'https://vkontakte.ru/share.php?url=%1$s&title=%2$s&description=%4$s&image=%3$a', $share ['url'], $share['title'], $share['image'], $share['description'] );
			}
			break;
		case 'ok' :
			//$url = sprintf ( 'http://www.odnoklassniki.ru/dk?st.cmd=addShare&st.s=1&st._surl=%1$s', $share ['url'] );
			$url = sprintf('https://connect.ok.ru/offer?url=%1$s&title=%2$s&imageUrl=%3$s', $share ['url'], $share['title'], $share['image']);
			break;
		case 'weibo' :
